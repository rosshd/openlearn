...
class CliStorageTests(unittest.TestCase):
    ...
    def start_legacy_placement(self, slug: str = "algorithms") -> None:
        activity = cli._begin_interview_activity(
            slug, lifecycle_version=cli.interview_prep.PLACEMENT_V1
        )
    ...
    def test_interview_prep_new_retry_cleans_precommit_profile_and_event_orphans(
        self,
    ) -> None:
        args = Namespace(
            topic="Algorithms",
        ...
        for stage in ("after_profile", "after_creation_event"):
            with self.subTest(stage=stage):
                def fail_at_stage(current: str) -> None:
                    if current == stage:
                        raise RuntimeError(f"simulated {stage} interruption")

                with mock.patch.object(
                    cli, "_new_course_checkpoint", side_effect=fail_at_stage
                ):
                    with self.assertRaisesRegex(RuntimeError, "simulated"):
                        cli.cmd_new(args, output_func=lambda _line: None)

                self.assertFalse(cli.topic_path("algorithms").exists())
                self.assertTrue(cli.interview_profile_path("algorithms").exists())
                if stage == "after_creation_event":
                    self.assertTrue(cli.topic_events_path("algorithms").exists())

                result = cli.cmd_new(args, output_func=lambda _line: None)

                self.assertEqual(result, 0)
                self.assertTrue(cli.topic_path("algorithms").exists())
                self.assertTrue(cli.interview_profile_path("algorithms").exists())
                events = cli.load_event_log(cli.topic_events_path("algorithms"))
                self.assertEqual(
                    sum(
                        event.get("event_type") == "interview_profile_created"
                        for event in events
                    ),
                    1,
                )
    ...
    def test_interview_prep_new_topic_is_visibility_marker_after_profile_and_event(
        self,
    ) -> None:
        ...
        def observe(stage: str) -> None:
            observed.append(
                (
                    stage,
                    cli.topic_path("algorithms").exists(),
                    cli.interview_profile_path("algorithms").exists(),
                    cli.topic_events_path("algorithms").exists(),
                )
            )

        with mock.patch.object(cli, "_new_course_checkpoint", side_effect=observe):
            cli.cmd_new(
                Namespace(
                    topic="Algorithms",
                    goal="Practice algorithms",
        ...
        self.assertIn(("after_profile", False, True, False), observed)
        self.assertIn(("after_creation_event", False, True, True), observed)
        self.assertIn(("after_topic", True, True, True), observed)

    def test_interview_prep_new_interruption_after_topic_is_fully_visible(
        self,
    ) -> None:
        args = Namespace(
            topic="Algorithms",
        ...
        def fail_after_topic(stage: str) -> None:
            if stage == "after_topic":
                raise RuntimeError("simulated post-commit interruption")

        with mock.patch.object(
            cli, "_new_course_checkpoint", side_effect=fail_after_topic
        ):
        ...
        with self.assertRaisesRegex(cli.OpenLearnError, "topic already exists"):
            cli.cmd_new(args, output_func=lambda _line: None)
    ...
    def test_interview_creation_and_multiline_interruptions_preserve_exact_state(
        self,
    ) -> None:
        default_profile_answers = [""] * len(cli.INTERVIEW_PROFILE_SETUP_FIELDS)
        cases = (
            ("confirmation", default_profile_answers, False, None, 0),
            (
                "post_create",
                [*default_profile_answers, "y"],
                True,
                "not_started",
                0,
            ),
        ...
        for interruption in (EOFError, KeyboardInterrupt):
            for phase, answers, published, status, evidence_count in cases:
                with self.subTest(interruption=interruption.__name__, phase=phase):
                    slug = f"{interruption.__name__}-{phase}".lower().replace("_", "-")
                    remaining = iter(answers)
                    output: list[str] = []
                    active_before = cli.get_active_topic()
                    ...
                    self.assertEqual(result, 0)
                    self.assertEqual(cli.topic_path(slug).exists(), published)
                    self.assertEqual(
                        cli.interview_profile_path(slug).exists(),
                        published,
                    )
                    if not published:
                        self.assertFalse(cli.topic_events_path(slug).exists())
                        self.assertEqual(cli.get_active_topic(), active_before)
                        self.assertIn("Nothing was saved", "\n".join(output))
                        continue
                    ...
                    self.assertEqual(placement["status"], status)
                    self.assertEqual(
                        len(placement["evidence_refs"]),
                        evidence_count,
                    )
    ...
    def test_interview_placement_cli_interrupts_resumes_and_stays_provisional(self) -> None:
        ...
        self.start_legacy_placement("algorithms")
        first_inputs = iter(["2", "Some Python experience", "/stop"])
    ...
    def test_interview_placement_status_uses_recorded_compact_lifecycle(self) -> None:
        ...
        value["placement"].pop("draft", None)
    ...
    def test_compact_v2_placement_can_continue_through_the_cli(self) -> None:
        slug = self.create_interview_topic("Compact V2 CLI")
        activity = cli._begin_interview_activity(
            slug, lifecycle_version=cli.interview_prep.PLACEMENT_V2
        )
        with cli.interview_profile_write_lock(slug):
            cli.interview_prep.start_placement(
                cli.interview_profile_path(slug),
                activity_id=str(activity["activity_id"]),
                lifecycle_version=cli.interview_prep.PLACEMENT_V2,
            )
        output: list[str] = []
        with (
            mock.patch.object(cli, "open_drill_in_editor", return_value="nvim"),
            ...
            cli.cmd_interview_placement(
                ...
                input_func=iter_input(
                    [
                        "2",
                        "I would clarify constraints and use a set window.",
                        "",
                        "I would improve it with last-seen indices.",
                    ]
                ),
                output_func=output.append,
            )
        ...
        self.assertEqual(completed["status"], "provisional")
        self.assertEqual(
            set(completed["observations"]),
            {"conversation", "implementation", "debrief"},
        )
        rendered = "\n".join(output)
        self.assertIn("Talk through the problem", rendered)
        self.assertIn("Debrief the solution", rendered)
    ...
    def test_reasoning_placement_continues_directly_when_provider_is_ready(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                cli, "_resume_interview_course_transition", return_value=17
            ) as transition,
        ):
            result = cli.cmd_interview_placement(
                ...
                input_func=iter_input(
                    [
                        ...
                        "/done",
                        "Use a sliding window and set, test edge cases, O(n) time and O(width) space.",
                        "/done",
                    ]
                ),
                output_func=output.append,
        ...
        self.assertIn("Starting your named first activity now", "\n".join(output))
    ...
    def test_reasoning_placement_providerless_completion_is_successful(self) -> None:
        ...
        self.assertEqual(result, 0)
        rendered = "\n".join(output)
        self.assertIn("Placement is complete and saved", rendered)
        self.assertIn("openlearn init", rendered)
        self.assertIn(f"openlearn resume {slug}", rendered)

    def test_active_legacy_placement_offers_safe_reasoning_migration(self) -> None:
        slug = self.create_interview_topic("Legacy Placement Migration")
        ...
        cli.cmd_interview_placement(
            Namespace(topic=slug, action="resume"),
            input_func=iter_input(["2", "I practice Python weekly.", "/stop"]),
            output_func=lambda _line: None,
        )
        old_activity = cli._current_interview_activity(slug)
        assert old_activity is not None
        old_activity_id = old_activity["activity_id"]
        old_evidence_ids = {
    ...
    def test_reasoning_done_recovers_after_activity_event_interruption(self) -> None:
        ...
        def fail_after_event(stage: str) -> None:
            if stage == "after_event":
                raise RuntimeError("simulated v3 publication interruption")

        cli._activity_update_checkpoint = fail_after_event
        try:
            with self.assertRaisesRegex(RuntimeError, "v3 publication interruption"):
                cli.cmd_interview_placement(
                    Namespace(topic=slug, action="resume"),
                    input_func=iter_input(["/done"]),
                    output_func=lambda _line: None,
                )
        finally:
        ...
        self.assertEqual(recovered["next_stage"], "reasoning")
        self.assertIsNone(recovered["draft"])
        self.assertEqual(len(recovered["evidence_refs"]), 1)
        evidence_events = [
            event
            for event in cli.load_event_log(cli.topic_events_path(slug))
            if event.get("event_type") == "activity_evidence_recorded"
    ...
    def test_reasoning_discard_recovers_after_activity_transition_interruption(
        self,
    ) -> None:
        ...
        with mock.patch.object(
            cli.interview_prep,
            "discard_placement",
            side_effect=RuntimeError("simulated discard interruption"),
        ):
            with self.assertRaisesRegex(RuntimeError, "discard interruption"):
                cli.cmd_interview_placement(
                    Namespace(topic=slug, action="discard"),
                    output_func=lambda _line: None,
                )

        interrupted = cli.interview_prep.load_profile(path)["placement"]
        self.assertEqual(interrupted["status"], "in_progress")
        activity = cli._current_interview_activity(slug)
        assert activity is not None
        self.assertEqual(activity["status"], "abandoned")

        recovered = cli.sync_interview_placement(slug)["placement"]
        self.assertEqual(recovered["status"], "not_started")
        discard_events = [
            event
            for event in cli.load_event_log(cli.topic_events_path(slug))
            if event.get("event_type") == "interview_placement_state_discarded"
        ]
        ...
        cli.cmd_interview_placement(
            ...
            input_func=iter_input(
                [
                    ...
                    "/done",
                    "PRIVATE PLAN RESPONSE using a hashmap; PRIVATE COMPLEXITY "
                    "RESPONSE; PRIVATE TEST RESPONSE",
                    "/done",
                ]
            ),
            output_func=lambda _line: None,
    ...
    def test_missing_provider_resume_is_contextual_and_non_mutating(self) -> None:
        ...
        self.assertIn("Placement: deferred (0/2)", transcript)
    ...
    def test_not_started_and_stale_resume_start_placement_before_provider(self) -> None:
        for initial_status in ("not_started", "stale"):
            with self.subTest(initial_status=initial_status):
                slug = f"{initial_status}-start".replace("_", "-")
                call_silent(
                    cli.cmd_new,
                    Namespace(
                        topic=slug,
    ...
    def test_not_started_and_stale_resume_defer_before_provider_and_skip_legacy_quiz(
        ...
        for initial_status in ("not_started", "stale"):
            with self.subTest(initial_status=initial_status):
                slug = f"{initial_status}-defer".replace("_", "-")
                call_silent(
                    cli.cmd_new,
                    Namespace(
                        topic=slug,
                ...
                def provider_ready() -> bool:
                    value = cli.interview_prep.load_profile(
                        cli.interview_profile_path(slug)
                    )
                    provider_states.append(str(value["placement"]["status"]))
                    return True

                with (
                    mock.patch.object(
                        cli, "_openlearn_mock_enabled", return_value=True
                    ),
                    mock.patch.object(
                        cli,
                        "provider_is_configured",
                        side_effect=provider_ready,
                    ),
                ):
                    result = cli.cmd_resume(
                        Namespace(topic=slug, model=None),
    ...
    def test_interview_resume_dry_run_does_not_recover_pending_profile_edit(
        ...
        cli._write_interview_edit_journal(
            "algorithms",
            profile_revision=1,
        ...
        self.assertTrue(cli.interview_edit_journal_path("algorithms").exists())
        self.assertIn("saved profile edit is pending", output.getvalue())

    def test_interview_resume_dry_run_does_not_enter_in_progress_placement(
        self,
    ) -> None:
        call_silent(
            cli.cmd_new,
        ...
        original_dry_run = cli._DRY_RUN
        cli._DRY_RUN = True
        try:
            result = cli.cmd_resume(
                Namespace(topic="algorithms", model=None),
                input_func=lambda _prompt: (_ for _ in ()).throw(
                    AssertionError("dry run must not enter placement")
                ),
                output_func=output.append,
            )
        finally:
            cli._DRY_RUN = original_dry_run
    ...
    def test_resume_refreshes_expired_provisional_placement_before_routing(self) -> None:
        ...
        cli.cmd_interview_placement(
            Namespace(topic="algorithms", action="resume"),
            input_func=iter_input(["2", "/baseline"]),
            output_func=lambda _line: None,
        )
        ...
        self.assertEqual(result, 0)
        self.assertEqual(profile["placement"]["status"], "stale")
        self.assertIn("Placement: stale", "\n".join(output))

    def test_expired_provisional_closes_old_activity_before_new_placement(
        self,
    ) -> None:
        call_silent(
            cli.cmd_new,
        ...
        with mock.patch.object(
            ...
            side_effect=RuntimeError("simulated transition interruption"),
        ):
            with self.assertRaisesRegex(RuntimeError, "transition interruption"):
                cli.cmd_interview_placement(
                    Namespace(topic="algorithms", action="resume"),
                    input_func=iter_input(
                        ["2", "Keep only the active streaming window."]
                    ),
                    output_func=lambda _line: None,
                )
        ...
        old_activity = cli._current_interview_activity("algorithms")
        old_attempt_id = interrupted["placement"]["attempt_id"]
        old_activity_id = old_activity["activity_id"]
        self.assertEqual(interrupted["placement"]["status"], "provisional")
        ...
        restarted = cli.interview_prep.load_profile(
            cli.interview_profile_path("algorithms")
        )
        new_activity = cli._current_interview_activity("algorithms")
    ...
    def test_resume_dry_run_projects_expired_staleness_without_writing(self) -> None:
        call_silent(
            cli.cmd_new,
            Namespace(
                topic="Algorithms",
        ...
        try:
            result = cli.cmd_resume(
                Namespace(topic="algorithms", model=None),
                input_func=lambda _prompt: (_ for _ in ()).throw(
                    AssertionError("dry run must not enter stale routing")
                ),
                output_func=output.append,
            )
        finally:
        ...
        self.assertEqual(snapshot_files(Path(self.home.name)), before)
        self.assertIn("Placement: stale", "\n".join(output))
        stored = cli.interview_prep.load_profile(
            cli.interview_profile_path("algorithms")
        )
    ...
    def test_interview_placement_recovers_activity_evidence_after_fault(self) -> None:
        ...
        try:
            with self.assertRaisesRegex(RuntimeError, "simulated process exit"):
                cli.cmd_interview_placement(
                    Namespace(topic="algorithms", action="resume"),
                    input_func=iter_input(["2", "I write Python weekly."]),
                    output_func=lambda _line: None,
                )
        finally:
            cli._activity_update_checkpoint = original_checkpoint
    ...
    def test_execution_pair_recovers_each_activity_publication_boundary(self) -> None:
        for boundary in ("after_journal", "after_state", "after_event"):
            with self.subTest(boundary=boundary):
                ...
                prompts: list[str] = []
                answers = iter(["2", "O(n) time.", "Use last-seen positions."])
                cli.cmd_interview_placement(
                    Namespace(topic=slug, action="resume"),
        ...
        for name, response, action in cases:
            with self.subTest(route=name):
                ...
                if response is None:
                    ...
                    cli.cmd_interview_placement(
                        Namespace(topic=slug, action=action),
                        input_func=iter_input(["2", response]),
                        output_func=lambda _line: None,
                    )
    ...
    def test_import_scan_deduplicates_by_checksum_and_reports_failures(self) -> None:
        ...
        original_call_openai = cli.call_openai
        original_extract_pdf = cli._extract_pdf_bytes
        cli.call_openai = lambda *_args, **_kwargs: "Summary."
        cli._extract_pdf_bytes = (
            lambda _data, _name, output_func=print: (_ for _ in ()).throw(
        ...
        try:
            ...
            cli.call_openai = original_call_openai
            cli._extract_pdf_bytes = original_extract_pdf
    ...
    def test_import_scan_persists_source_folder_provenance(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        source = directory / "week1.txt"
        source.write_text("lecture one", encoding="utf-8")
        ...
        folders = cli.imported_source_folders(cli.read_topic("ai").metadata)
        record = folders[str(directory.resolve())]["files"]["week1.txt"]
        self.assertEqual(record["source_path"], str(source.resolve()))
        self.assertEqual(record["context_file"], "week1.txt")
        self.assertEqual(record["summary_file"], "week1.txt.summary.txt")
        self.assertEqual(record["checksum"], cli._file_checksum(source))

    def test_folder_refresh_unchanged_preserves_artifacts_and_avoids_provider(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        source = directory / "week1.txt"
        source.write_text("lecture one", encoding="utf-8")
        with mock.patch.object(cli, "generate_context_summary", return_value="Summary one.\n"):
            call_silent(cli.cmd_import_scan, "ai", directory, model="test-model")
        context = cli.topic_context_dir("ai") / "week1.txt"
        summary = cli.topic_context_dir("ai") / "week1.txt.summary.txt"
        topic_before = cli.topic_path("ai").stat().st_mtime_ns
    ...
    def test_folder_refresh_updates_only_changed_file(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        first = directory / "week1.txt"
        second = directory / "week2.md"
        first.write_text("lecture one", encoding="utf-8")
        second.write_text("# lecture two", encoding="utf-8")
        ...
        with mock.patch.object(
            cli, "generate_context_summary", return_value="Revised summary.\n"
        ) as generate:
            result = cli.refresh_imported_source_folders(
        ...
        self.assertEqual(
            (cli.topic_context_dir("ai") / "week1.txt").read_text(encoding="utf-8"),
            "lecture one revised\n",
        )
        self.assertEqual(
            (cli.topic_context_dir("ai") / "week1.txt.summary.txt").read_text(
                encoding="utf-8"
            ),
            "Revised summary.\n",
        )
        self.assertEqual(second_context.stat().st_mtime_ns, second_context_mtime)
        self.assertEqual(second_summary.stat().st_mtime_ns, second_summary_mtime)
        self.assertEqual(first.read_text(encoding="utf-8"), "lecture one revised")

    def test_folder_refresh_imports_new_supported_file_incrementally(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        first = directory / "week1.txt"
        ...
        with mock.patch.object(cli, "generate_context_summary", return_value="Initial.\n"):
            call_silent(cli.cmd_import_scan, "ai", directory, model="test-model")
        first_context = cli.topic_context_dir("ai") / "week1.txt"
        first_mtime = first_context.stat().st_mtime_ns
        (directory / "week2.md").write_text("# lecture two", encoding="utf-8")

        with mock.patch.object(cli, "generate_context_summary", return_value="New.\n"):
            result = cli.refresh_imported_source_folders(
                "ai", model="test-model", output_func=lambda _text: None
            )

        self.assertEqual(result, (1, 1, 0))
        self.assertEqual(first_context.stat().st_mtime_ns, first_mtime)
        folders = cli.imported_source_folders(cli.read_topic("ai").metadata)
        self.assertEqual(
            set(folders[str(directory.resolve())]["files"]),
            {"week1.txt", "week2.md"},
        )

    def test_folder_refresh_unavailable_is_clear_and_non_corrupting(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        ...
        with mock.patch.object(cli, "generate_context_summary", return_value="Summary.\n"):
            call_silent(cli.cmd_import_scan, "ai", directory, model="test-model")
        topic_before = cli.topic_path("ai").read_bytes()
        context_before = {
            path.name: path.read_bytes() for path in cli.context_files("ai")
        }
        directory.rename(Path(self.home.name) / "semester-moved")
        output: list[str] = []
        ...
        self.assertIn("folder unavailable", output[0])
        self.assertEqual(cli.topic_path("ai").read_bytes(), topic_before)
        self.assertEqual(
            {path.name: path.read_bytes() for path in cli.context_files("ai")},
            context_before,
        )

    def test_folder_refresh_provider_failure_preserves_previous_artifacts(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        source = directory / "week1.txt"
        ...
        source.write_text("lecture one revised", encoding="utf-8")
        output: list[str] = []

        with mock.patch.object(
            cli,
            "generate_context_summary",
            side_effect=cli.OpenLearnError("provider unavailable"),
        ):
            result = cli.refresh_imported_source_folders(
                "ai", model="test-model", output_func=output.append
            )
        ...
        self.assertEqual(result, (0, 0, 1))
        self.assertIn("provider unavailable", output[0])
        self.assertEqual(cli.topic_path("ai").read_bytes(), topic_before)
        self.assertEqual(
            {path.name: path.read_bytes() for path in cli.context_files("ai")},
            context_before,
    ...
    def test_initial_folder_summary_failure_leaves_no_orphan_artifact(self) -> None:
        ...
        self.assertIn("0 imported, 0 skipped (already imported), 1 failed", output)
        self.assertEqual(cli.context_files("ai"), [])
        self.assertEqual(cli.imported_source_folders(cli.read_topic("ai").metadata), {})

    def test_initial_folder_provenance_failure_rolls_back_generated_artifacts(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        (directory / "week1.txt").write_text("lecture one", encoding="utf-8")
        ...
        with (
            mock.patch.object(
                cli, "generate_context_summary", return_value="Summary.\n"
            ),
            mock.patch.object(
                cli,
                "save_imported_source_provenance",
                side_effect=cli.OpenLearnError("metadata unavailable"),
            ),
        ):
            output = capture_stdout(
                cli.cmd_import_scan,
    ...
    def test_folder_flows_reject_file_swapped_to_outside_symlink_before_open(self) -> None:
        ...
        def swap_during_final_open(source: Path):
            swapped = False

            def guarded_open(path, flags, mode=0o777, *, dir_fd=None):
                nonlocal swapped
                if dir_fd is not None and path == source.name and not swapped:
                    swapped = True
                    source.unlink()
        ...
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        scan_directory = Path(self.home.name) / "scan-semester"
        scan_directory.mkdir()
        scan_source = scan_directory / "scan.txt"
        scan_source.write_text("safe scan notes", encoding="utf-8")
        with (
            mock.patch.object(
                cli.os, "open", side_effect=swap_during_final_open(scan_source)
            ),
            mock.patch.object(cli, "generate_context_summary") as generate,
        ):
            output = capture_stdout(
                cli.cmd_import_scan,
                "ai",
        ...
        setup_directory = Path(self.home.name) / "setup-semester"
        setup_directory.mkdir()
        setup_source = setup_directory / "setup.txt"
        setup_source.write_text("safe setup notes", encoding="utf-8")
        with (
            mock.patch.object(
                cli.os, "open", side_effect=swap_during_final_open(setup_source)
            ),
            mock.patch.object(cli, "pending_context_from_snapshot") as read_snapshot,
        ):
            contexts = cli.pending_contexts_from_dir(
                setup_directory, output_func=lambda _text: None
            )
        read_snapshot.assert_not_called()
        self.assertEqual(contexts, [])

        refresh_directory = Path(self.home.name) / "refresh-semester"
        refresh_directory.mkdir()
        refresh_source = refresh_directory / "refresh.txt"
        refresh_source.write_text("safe refresh notes", encoding="utf-8")
        with mock.patch.object(cli, "generate_context_summary", return_value="Initial.\n"):
            call_silent(
                cli.cmd_import_scan,
                "ai",
        ...
        topic_before = cli.topic_path("ai").read_bytes()
        refresh_source.write_text("revised safe notes", encoding="utf-8")
        with (
            mock.patch.object(
                cli.os, "open", side_effect=swap_during_final_open(refresh_source)
            ),
            mock.patch.object(cli, "generate_context_summary") as generate,
        ):
            result = cli.refresh_imported_source_folders(
        ...
        self.assertNotIn(
            "must never reach generated context",
            "\n".join(
                path.read_text(encoding="utf-8") for path in cli.context_files("ai")
            ),
        )
    ...
    def test_same_stem_folder_sources_keep_distinct_summaries_through_refresh(self) -> None:
        ...
        text_summary = context_dir / "lesson.txt.summary.txt"
    ...
    def test_new_folder_summary_does_not_overwrite_legacy_provenance_name(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        directory = Path(self.home.name) / "semester"
        directory.mkdir()
        markdown_source = directory / "foo.md"
        markdown_source.write_text("# legacy markdown", encoding="utf-8")
        legacy_context = cli.write_context_text("ai", "foo.md", "# legacy markdown")
        legacy_summary = cli.topic_context_dir("ai") / "foo.summary.txt"
        legacy_summary.write_text("Legacy summary must remain.\n", encoding="utf-8")
        cli.save_imported_source_provenance(
            "ai",
            directory,
            markdown_source,
            legacy_context,
            legacy_summary,
            cli._file_checksum(markdown_source),
        )
        text_source = directory / "foo.txt"
        text_source.write_text("new text source", encoding="utf-8")

        with mock.patch.object(
            cli, "generate_context_summary", return_value="New text summary.\n"
        ):
            result = cli.refresh_imported_source_folders(
                "ai", model="test-model", output_func=lambda _text: None
            )
        ...
        new_summary_name = records["foo.txt"]["summary_file"]
        self.assertEqual(new_summary_name, "foo.txt.summary.txt")
        self.assertEqual(
            (cli.topic_context_dir("ai") / new_summary_name).read_text(encoding="utf-8"),
            "New text summary.\n",
        )
        self.assertEqual(records["foo.md"]["summary_file"], "foo.summary.txt")
    ...
    def test_course_setup_preserves_folder_source_provenance(self) -> None:
        ...
        source = directory / "week1.txt"
        source.write_text("lecture one", encoding="utf-8")
        contexts = cli.pending_contexts_from_dir(directory, output_func=lambda _text: None)

        cli.create_course_from_setup(
            "AI",
            "learn ai",
            contexts,
        ...
        folders = cli.imported_source_folders(cli.read_topic("ai").metadata)
        self.assertEqual(
            folders[str(directory.resolve())]["files"]["week1.txt"]["source_path"],
            str(source.resolve()),
        )

    def test_resume_checks_imported_folders_before_building_context(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
...
class InteractiveTests(unittest.TestCase):
    ...
    def test_ask_topic_judge_failure_preserves_pending_question_and_stops_generation(
        ...
        with (
            ...
            with self.assertRaisesRegex(cli.OpenLearnError, "grade"):
                cli.ask_topic(
                    "python",
        ...
        self.assertEqual(updated.metadata["last_answer_status"], "")
        self.assertIn("pending_question", updated.metadata)
        self.assertNotIn("It returns a value to the caller.", updated.body)
    ...
    def test_incomplete_answer_judgment_does_not_reuse_stale_answer_state(self) -> None:
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(cli.OpenLearnError, "incomplete"):
    ...
    def test_multiple_choice_answer_key_parses_only_explicit_selections(self) -> None:
        question = (
            "Which keyword sends a value back?\n"
            "A) print\nB) return\nC) def\nD) pass"
        )
        for answer in (
            "B",
            "Option B",
            "I think B because it returns the result.",
            "The answer is B",
            "return",
        ):
        ...
        for answer in (
            "A return value goes back to the caller.",
            "Because return sends the result back.",
            "I am deciding between A and B.",
            "Option B or C",
            "I think B or C",
            "The answer is B, but maybe C",
            "Choice B, though possibly C",
        ):
            with self.subTest(answer=answer):
                self.assertIsNone(
                    cli.explicit_multiple_choice_option(answer, question)
    ...
    def test_multiple_choice_free_text_keeps_complete_semantic_judgment(self) -> None:
        ...
        self.assertEqual(result, "answer")
        self.assertEqual(updated.metadata["last_answer_status"], "correct")
        self.assertEqual(updated.metadata["last_answer_score"], 0.9)
        self.assertNotIn("pending_question", updated.metadata)

    def test_multiple_choice_key_override_uses_shared_explicit_parser(self) -> None:
        question = (
            "Which keyword sends a value back?\n"
            "A) print\nB) return\nC) def\nD) pass"
        )
        for answer, expected_status in (
            ("Option B", "correct"),
            ("I think B because it returns the result.", "correct"),
            ...
            with self.subTest(answer=answer):
                ...
                self.assertTrue(
                    cli.prepare_current_answer_judgment(metadata, answer, update)
                )
                self.assertEqual(update["last_answer_status"], expected_status)
                cli.apply_pending_question_answer_key(metadata, answer)
                self.assertEqual(metadata["last_answer_status"], expected_status)

    def test_ask_topic_judge_outage_fails_before_tutor_and_retries_once(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="Python", goal="Learn functions"))
        path = cli.topic_path("python")
        metadata, body = cli.parse_topic(path.read_text(encoding="utf-8"))
        metadata = dict(metadata)
        ...
        with (
            mock.patch.object(
                cli,
                "call_openai",
                side_effect=cli.OpenLearnError("judge offline"),
            ),
            mock.patch.object(cli, "call_openai_streaming", tutor),
        ):
            with self.assertRaisesRegex(cli.OpenLearnError, "grade"):
        ...
        self.assertEqual(pending["question"], "What does a return value do?")
        ...
        judge = mock.Mock(return_value=json.dumps(judgment))
        with (
            mock.patch.object(cli, "call_openai", judge),
            mock.patch.object(cli, "call_openai_streaming", tutor),
            mock.patch.object(cli, "maybe_suggest_videos"),
        ):
            cli.ask_topic(
    ...
    def test_ask_topic_unusable_judgments_fail_before_tutor_and_retry_once(
        ...
        for case, unusable_result in unusable_results.items():
            with self.subTest(case=case):
                topic_name = f"Python {case}"
                slug = cli.slugify(topic_name)
                call_silent(
                    cli.cmd_new,
                    Namespace(topic=topic_name, goal="Learn functions"),
                )
                path = cli.topic_path(slug)
                metadata, body = cli.parse_topic(path.read_text(encoding="utf-8"))
                metadata = dict(metadata)
                ...
                answer = f"It sends the {case} result back to the caller."
                ...
                tutor.assert_not_called()
                self.assertEqual(cli.load_pending_learner_prompt(slug), answer)
                self.assertEqual(
                    cli.read_topic(slug).metadata["pending_question"]["question"],
                ...
                judge.assert_called_once()
                tutor.assert_called_once()
                self.assertIsNone(cli.load_pending_learner_prompt(slug))
                attempts = cli.read_topic(slug).metadata["concept_attempts"]
                self.assertEqual(attempts["return-values"]["attempts"], 1)
                judged_events = [
                    event
                    for event in cli.load_event_log(cli.topic_events_path(slug))
    ...
    def test_keyboard_interrupt_during_tutor_stream_rolls_back_for_retry(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="Python", goal="Learn functions"))
        path = cli.topic_path("python")
        metadata, body = cli.parse_topic(path.read_text(encoding="utf-8"))
        metadata = dict(metadata)
        ...
        answer = "It sends a result to the caller."
        cli.save_pending_learner_prompt("python", answer)
        before = cli.read_topic("python")
        events_before = cli.load_event_log(cli.topic_events_path("python"))
        judgment = {
        ...
        restored = cli.read_topic("python")
        self.assertEqual(
            restored.metadata["pending_question"],
            before.metadata["pending_question"],
        )
        self.assertEqual(
            restored.metadata["last_answer_status"],
            before.metadata["last_answer_status"],
        )
        self.assertNotIn("concept_attempts", restored.metadata)
        self.assertEqual(cli.load_pending_learner_prompt("python"), answer)
        self.assertEqual(
...
class PromptInstructionTests(unittest.TestCase):
    ...
    def test_review_grades_only_five_deterministically_selected_due_concepts(self) -> None:
        call_silent(cli.cmd_new, Namespace(topic="AI", goal="learn ai"))
        path = cli.topic_path("ai")
        metadata, body = cli.parse_topic(path.read_text(encoding="utf-8"))
        metadata = dict(metadata)
        yesterday = (date.fromisoformat(cli.today()) - timedelta(days=1)).isoformat()
        concepts = [
            ("concept-c", cli.today(), [3.0, 3.0, 3.0]),
            ("concept-a", yesterday, [1.0, 1.0, 1.0]),
            ("concept-g", cli.today(), [7.0, 7.0, 7.0]),
            ("concept-b", yesterday, [2.0, 2.0, 2.0]),
            ("concept-f", cli.today(), [6.0, 6.0, 6.0]),
            ("concept-e", cli.today(), [5.0, 5.0, 5.0]),
            ("concept-d", cli.today(), [4.0, 4.0, 4.0]),
        ]
        metadata["review_due"] = [
            {
                "concept": concept,
            ...
            for concept, due, model in concepts
        ]
        path.write_text(cli.format_topic(metadata, body), encoding="utf-8")
        update_calls = []
        captured_prompts = []
        ...
        for concept in selected:
            self.assertIn(f"- {concept}", updated.body)
        for concept in unselected:
            self.assertNotIn(concept, updated.body)
        for concept in selected:
            self.assertEqual(review_items[concept]["last_reviewed"], cli.today())
        for concept, due, model in concepts:
            if concept not in unselected:
                continue
            self.assertEqual(review_items[concept]["due"], due)
            self.assertEqual(review_items[concept]["difficulty"], "hard")
            self.assertEqual(review_items[concept]["ebisu_model"], model)
            self.assertNotIn("last_reviewed", review_items[concept])
        ...
        self.assertEqual(
            [event["data"]["concept"] for event in graded_events],
            selected,
        )
        self.assertTrue(
            unselected.isdisjoint(
                event["data"]["concept"] for event in graded_events
            )
        )
    ...
    def test_first_lesson_prompt_avoids_filler_questions(self) -> None:
        ...
        interview_prompt = cli.first_lesson_prompt(
...
def make_interview_placement_expired(slug: str) -> None:
    path = cli.interview_profile_path(slug)
    value = json.loads(path.read_text(encoding="utf-8"))
    expired = (datetime.now(timezone.utc) - timedelta(days=91)).isoformat()
    value["placement"]["completed_at"] = expired
    value["placement"]["updated_at"] = expired
