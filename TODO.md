...
- [x] **Session resilience** - transient provider failures retry with bounded backoff, in-flight REPL answers survive process restart for Enter resubmission or typed replacement, and `openlearn repair` fixes simple corrupt JSON frontmatter with `.bak` backup
