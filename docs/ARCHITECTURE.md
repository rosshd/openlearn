...
Pytest collects the repository's `unittest.TestCase` suites, so the gate executes each test once.
GitHub Actions runs pytest on Ubuntu, Windows, and macOS for Python 3.11 through 3.13.
